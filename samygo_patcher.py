...
#version = '0.35' #ReWrtten For Python3 (?)FrankSX
    ...
    try:
        ...
        while bytesToCopy:
            ...
            if bytesToCopy >= len(keyData):
                ...
                encryptedData = ''.join([chr(ord(data[i])^ord(keyData[i])) for i in range(len(data))])
...
def patch_Telnet( FileTarget ):
    ...
    var = input("Enable Telnet or Advanced Mode or nothing on image( T/a/n )? ")
...
def patch_TelnetRBYDEUC( FileTarget ):
    ...
    while ifile.tell() < FileSize:
        ...
        if found != -1 :
            ...
            var = input("Enable starting script SamyGO/rcSGO from USB or no change ( U/n )? ")
...
def GetSquashFSToolsPath():
    ...
    if not os.path.exists( toolspack ):
        ...
        while 1:
            try:
                tst = urllib.request.urlopen( address )
                break
            except IOError:
        ...
        if tst.getcode() == 200:
            urllib.request.urlretrieve( address , toolspack )
            tst.close()
    ...
    elif sys.platform=='linux2' and sys.maxsize==2**31-1: #sys.arch='i386'
        os.system('chmod +x squashfs-tools/linux-x86/*')
        return 'squashfs-tools/linux-x86'

    elif sys.platform=='linux2' and sys.maxsize==2**63-1: #sys.arch='x86_64'
...
def Patch_Wiselink_Player_Hack( exeDSPFileName ):
    ...
    if patch_check == "\x24\x01\x00":
        ...
        var = input("Enable Wiselink Player Hack ( Y/n )? ")
        if var == 'n' or var == 'N':
...
def Patch_VideoAR_v1_Fix( exeDSPFileName ):
    ...
    if patch_check == "\x01\x02\x01\x02\x01":
        ...
        var = input("Enable VideoAR Fix v1 ( Y/n )? ")
        if var == 'n' or var == 'N':
...
def Patch_Big_Subtitles( exeDSPFileName ):
    ...
    if patch_check == "\x18\x18":
        ...
        var = input("Enable Big Subtitles ( Y/n )? ")
        if var == 'n' or var == 'N':
    ...
    if Color == 0xFFF0F0F0:
        ...
        var = input("Want to Change Substitle Color ( y/N )? ")
        if var == 'y' or var == 'Y':
            while True:
                try:
                    var = input("Enter New Color Value as ARGB (default 0xFFF0F0F0) : 0x")
                    if len(var) !=8:
                        continue
                    Color=int(var,16)
